...
export async function run(
  ...
  const analysis: Analysis = 'question' in input ? input : { question: input.content || '' };
...
async function gatherPerspectives(
  ...
  if (verbose) {
    console.log('Phase 1: Gathering perspectives from all six hats...\n');
  }
...
async function synthesize(
  ...
  const perspectivesText = perspectives.map((p) => `${p.name}:\n${p.analysis}\n`).join('\n---\n\n');
