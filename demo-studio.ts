...
import {
  ...
  getDefaultConfig,
  formatResult,
} from './studio-critique-poc/src/orchestrator';
...
Skateboarding figured this out. The rest of culture is still learning.`,
    context:
      "This essay explores the tension between mainstream acceptance and underground authenticity in skateboarding. I'm using Nike SB as a case study to show how authenticity isn't about purity but about cultural respect. Target audience is both skaters and people interested in how subcultures handle growth.",
    medium: 'writing',
    stage: 'draft',
  };

  // Setup provider (throws clearly if ANTHROPIC_API_KEY is unset)
  const provider = new AnthropicProvider(getAPIKey('anthropic'));
