...
describe('CircuitBreakerProvider', () => {
  ...
  test('opens after consecutive failures reach threshold', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        await cb.call({ model: 'm', messages: [{ role: 'user', content: 'x' }] });
      } catch {}
    }
  ...
  test('reopens on failed probe in half-open state', async () => {
    ...
    await Bun.sleep(60);
    try {
      await cb.call({ model: 'm', messages: [{ role: 'user', content: 'probe' }] });
    } catch {}
    expect(cb.state).toBe('open');
  });
  ...
  test('resets failure count on success', async () => {
    ...
    try {
      await cb.call({ model: 'm', messages: [{ role: 'user', content: '2' }] });
    } catch {}
